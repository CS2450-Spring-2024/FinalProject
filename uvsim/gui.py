...
class App(tk.Tk):
    def __init__(self, screenName: str | None = None, baseName: str | None = None, className: str = "Tk", useTk: bool = True, sync: bool = False, use: str | None = None) -> None:
        ...
        self.label = tk.Label(self, text="Hello, GUI!", font=('Arial', 18))
        ...
        # left side menu panel open, save, exit, run, etc.
        self.menu_frame = tk.Frame(self.master_frame, bg="light green")
        self.menu_frame.grid(row=0, rowspan=2, column=0, sticky="news", padx=5, pady=5)
        ...
        # Button widgets 
        self.run_one = tk.Button(self.menu_frame, text="Run One Instruction", font=('Arial', 18)) # Create a button widget     
        self.run_one.grid(row=0, column=0, sticky="news") # Place the button in the frame

        self.run_until_address = tk.Button(self.menu_frame, text="Run Until Address", font=('Arial', 18)) # Create a button widget     
        self.run_until_address.grid(row=1, column=0, sticky="news") # Place the button in the frame

        self.run_until_halt = tk.Button(self.menu_frame, text="Run Until Halt", font=('Arial', 18)) # Create a button widget     
        self.run_until_halt.grid(row=2, column=0, sticky="news") # Place the button in the frame

        self.exit = tk.Button(self.menu_frame, text="Exit", font=('Arial', 18)) # Create a button widget     
        self.exit.grid(row=3, column=0, sticky="news") # Place the button in the frame
        
        self.pause = tk.Button(self.menu_frame, text="Pause", font=('Arial', 18)) # Create a button widget     
        self.pause.grid(row=4, column=0, sticky="news") # Place the button in the frame
        
        self.reset = tk.Button(self.menu_frame, text="Reset", font=('Arial', 18)) # Create a button widget     
        self.reset.grid(row=5, column=0, sticky="news") # Place the button in the frame  
        # End Button widgets  
        ...
        self.top_frame.columnconfigure(0, weight=1) # Set the column weight to 1   
        self.top_frame.columnconfigure(1, weight=1) # Set the column weight to 1   
        self.top_frame.columnconfigure(2, weight=1) # Set the column weight to 1 
        self.top_frame.rowconfigure(0, weight=1)
        
                    #________ Top Left Frame Start ________
        self.open_section = tk.Frame(self.top_frame, bg="red")
        self.label_top_left = tk.Label(self.open_section, text="Updates Submission", font=('Arial', 18))
        self.label_top_left.pack()
        self.submit_button = tk.Button(self.open_section, text="Submit Change", font=('Arial', 18)) # Create a button widget 
        ...
        self.label_top_mid = tk.Label(self.console_section, text="Console Output", font=('Arial', 18))
        self.label_top_mid.pack()
        self.run_in_entry = tk.Entry(self.console_section, font=('Arial', 18))
        ...
        self.label_top_right = tk.Label(self.accum_section, text="Accumulator", font=('Arial', 18))
